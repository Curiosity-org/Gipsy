...
class Misc(commands.Cog):
    ...
    async def cookie(self, ctx: MyContext, *, user: discord.User = None):
        ...
        if user:
            message = I18N.get(ctx.guild, 'misc.cookie.give', to=user.mention, giver=ctx.author.mention)
        else:
            message = I18N.get(ctx.guild, 'misc.cookie.self', to=ctx.author.mention)
    ...
    # async def get(self, ctx: MyContext, snowflake = None):
