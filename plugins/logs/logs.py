...
class Logs(commands.Cog):
    ...
    async def modlogs_enable(
        ...
        await SERVER_CONFIG.edit_config(
            ctx.guild.id, "modlogs_flags", LogsFlags.flagsToInt(flags)
        )
        await ctx.send(
            await self.bot._(
    ...
    async def modlogs_list(self, ctx: MyContext):
        ...
        await ctx.send(
            await self.bot._(ctx.guild.id, "sconfig.modlogs-list", list=" - ".join(f))
        )
    ...
    async def on_message_edit(self, before: discord.Message, after: discord.Message):
        ...
        _footer = await self.bot._(
            before.guild.id, "logs.footer1", author=before.author.id, message=before.id
        )
        embed.set_footer(text=_footer)
    ...
    async def on_invite_create(self, invite: discord.Invite):
        ...
        if invite.inviter:  # sometimes Discord doesn't send that info
            embed.set_author(
                ...
                icon_url=invite.inviter.display_avatar.url_as(static_format="png"),
            )
            _footer = await self.bot._(
    ...
    async def on_member_join_remove(self, member: discord.Member, join: bool):
        ...
        if join:
            embed = discord.Embed(
                ...
                description=await self.bot._(
                    member.guild.id, "logs.member_join.desc", user=member.mention
                ),
                colour=discord.Colour.green(),
            )
            ...
            embed = discord.Embed(
                ...
                description=await self.bot._(
                    member.guild.id, "logs.member_left.desc", user=member.mention
                ),
    ...
    async def on_voice_state_update(
        ...
        embed = discord.Embed(
            title=_title,
            description=await self.bot._(
                member.guild.id, "logs.voice_move." + _desc, user=member.mention, **kw
            ),
        )
    ...
    async def on_guild_role_create(self, role: discord.Role):
        ...
        if role.color != discord.Color.default():
            data.append(
                await self.bot._(role.guild.id, "logs.role_created.color")
                + f" {role.color}"
            )
        if role.hoist:
        ...
        if before.roles != after.roles:
            ...
            if got:
                _added = await self.bot._(
                    after.guild.id, "logs.member_update.roles_added", count=len(got)
                )
                embed.add_field(name=_added, value=" ".join(got), inline=False)
            if lost:
                _removed = await self.bot._(
                    after.guild.id, "logs.member_update.roles_removed", count=len(lost)
                )
                embed.add_field(name=_removed, value=" ".join(lost), inline=False)
        if len(embed.fields) == 0:
            return
        embed.set_author(
            name=str(before), icon_url=before.display_avatar.url_as(static_format="png")
        )
    ...
    async def on_guild_emojis_update(
        ...
        if restrict:
            n = await self.bot._(
                guild.id, "logs.emoji_update.restrict", count=len(restrict)
            )
