    ...
    async def ban(self, ctx: MyContext, *, user: nextcord.User, reason: str = None):
        ...
        if ctx.guild.id != "125723125685026816":
            ...
            if random.randint(1, 10) == 1:
                try:
                    await ctx.guild.ban(ctx.author, delete_message_days=0, reason=f"Banned by himself. Reason : {user} ({user.id}) used Uno Reverse card.")
                except nextcord.Forbidden:
                    await ctx.send("Permissions manquantes :confused: (vérifiez la hiérarchie)")
                else:
            ...
            if random.randint(1, 10) == 1:
                msg = await self.bot._(ctx.channel, f"ban.gunivers.rickroll")
            ...
            if ctx.author.id == "125722240896598016":
                try:
                    ...
                    msg = await self.bot._(ctx.channel, f"ban.gunivers.ban.{choice}")
                    await ctx.send(msg.format(ctx.author.mention, user.mention))
                    await ctx.send("https://media.discordapp.net/attachments/791335982666481675/979052868915064862/Chauve_qui_peut_.png")
